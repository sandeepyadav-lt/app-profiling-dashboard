...
import { coldStartupData, coldStartupStats } from '../../data/cold-startup-data';
...
import styles from './ColdStartupWidget.module.css';

const baseValues = coldStartupData.map((d) => d.value);
const labels = coldStartupData.map((d) => d.label);

const defaultChartData = {
  labels,
  datasets: [
    {
      label: 'Cold Startup (ms)',
      data: baseValues,
...
const options: ChartOptions<'line'> = {
  ...
  scales: {
    ...
    y: {
      ...
      title: { display: true, text: 'Cold Startup (ms)' },
    },
  },
};

export function ColdStartupWidget({ instance, index }: WidgetInstanceProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {isComparing ? (
          <CompareStatsTable
            columns={[{ label: 'Cold Startup', unit: ' ms' }]}
          ...
          <StatsRow stats={coldStartupStats} />
        )}
      </WidgetCard>
      <FilterPanel
