...
import { batteryTempData, batteryTempStats } from '../../data/battery-temperature-data';
...
import styles from './BatteryTemperatureWidget.module.css';

const baseValues = batteryTempData.map((d) => d.value);
const labels = batteryTempData.map((d) => d.label);
...
export function BatteryTemperatureWidget({ instance, index }: WidgetInstanceProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {isComparing ? (
          <CompareStatsTable
            columns={[{ label: 'Temperature', unit: '\u00B0C' }]}
          ...
          <StatsRow stats={batteryTempStats} />
        )}
      </WidgetCard>
      <FilterPanel
