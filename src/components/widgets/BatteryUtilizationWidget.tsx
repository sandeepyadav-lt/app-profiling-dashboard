...
import { batteryChartData, batteryStats } from '../../data/battery-utilization-data';
...
import styles from './BatteryUtilizationWidget.module.css';

const baseValues = batteryChartData.map((d) => d.value);
const labels = batteryChartData.map((d) => d.label);
...
export function BatteryUtilizationWidget({ instance, index }: WidgetInstanceProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {isComparing ? (
          <CompareStatsTable
            columns={[{ label: 'Battery', unit: ' mAh' }]}
